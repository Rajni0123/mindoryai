...
class ProfessionalManimRenderer:
    ...
    def render_scene(
        ...
        # Generate images for each key concept
        concept_images = []
        concepts_to_use = concepts[:4]  # max 4 concept images per scene (visual-first)
        for i, concept in enumerate(concepts_to_use):
            clean_concept = str(concept).strip()
            if len(clean_concept) < 2:
                continue

            img_path = os.path.join(img_dir, f"scene_{scene_number}_c_{i}.png")
            img = self.img_gen.generate_concept_image(
    ...
    def _build_scene(self, scene_number, title, concept_images, duration, narration=""):
        ...
        outline_colors = ["'#FF6B6B'", "'#4ECDC4'", "'#45B7D1'", "'#96CEB4'", "'#FFEAA7'", "'#DDA0DD'"]
        ...
        # ── Build image definitions (all positioned in grid) ──
        img_defs = ""
        ...
        try:
            ...
            highlight_{i} = SurroundingRectangle(
                cimg_{i}, color={outline_color}, stroke_width=4, buff=0.08,
                corner_radius=0.1
            )
        except Exception:
            cimg_{i} = None
...
"""
        ...
        if n_images > 0:
            ...
            for i in range(min(n_images, 4)):
                fade_in_time = min(time_per_img * 0.35, 0.8)
                label_time = min(time_per_img * 0.15, 0.4)
                view_time = max(time_per_img - fade_in_time - label_time - 0.3, 0.3)

                cap_text = ""
                if i < len(captions):
                    cap_text = captions[i].replace('"', '\\"').replace("'", "\\'")

                px, py = positions[i] if i < len(positions) else (0.0, 0.0)
                label_y = py - (img_max_h / 2) - 0.25

                # Fade in image + outline (stays on screen)
                anim_code += f"""
        # ── Image {i+1} (stays on screen) ──
        if cimg_{i}:
            self.play(FadeIn(cimg_{i}, shift=UP * 0.2), run_time={fade_in_time:.2f})
            if highlight_{i}:
                self.play(Create(highlight_{i}), run_time=0.2)
"""
                # Short label below image
                if cap_text:
                    anim_code += f"""
            lbl_{i} = Text("{cap_text}", font="{self.body_font}", font_size=16, color=BLACK)
            lbl_{i}.move_to([{px}, {label_y:.1f}, 0])
            if lbl_{i}.width > {img_max_w - 0.3}:
                lbl_{i}.scale({img_max_w - 0.3} / lbl_{i}.width)
            self.play(FadeIn(lbl_{i}), run_time={label_time:.2f})
"""

                # Arrow connecting this image to the next one
                if i < min(n_images, 4) - 1:
                    nx, ny = positions[i + 1] if (i + 1) < len(positions) else (0.0, 0.0)
                    # Arrow from current image edge to next image edge
                    if n_images <= 3:
                        # horizontal arrows for row layout
...
"""
                        ...
                        # 2x2 grid: arrows follow reading order (1→2, 2→3 diagonal, 3→4)
                        if i == 0:  # top-left → top-right
                            ax1, ay1 = px + img_max_w / 2 + 0.1, py
                            ax2, ay2 = nx - img_max_w / 2 - 0.1, ny
                        elif i == 1:  # top-right → bottom-left (diagonal)
...
"""
            ...
            # No images fallback: show captions as text
            time_per_cap = anim_time / max(len(captions), 1)
            for ci, cap in enumerate(captions):
                cap_esc = cap.replace('"', '\\"').replace("'", "\\'")
                write_time = min(time_per_cap * 0.5, 1.0)
                view_time = max(time_per_cap - write_time - 0.2, 0.3)
                y_pos = 1.5 - ci * 1.2

                anim_code += f"""
        text_{ci} = Text("{cap_esc}", font="{self.body_font}", font_size=26, color=BLACK)
        text_{ci}.move_to([0.0, {y_pos:.1f}, 0])
        if text_{ci}.width > 12.0:
            text_{ci}.scale(12.0 / text_{ci}.width)
        self.play(Write(text_{ci}), run_time={write_time:.2f})
        self.wait({view_time:.2f})
"""
...
"""
    ...
    def _run_manim(self, scene_file: str, scene_class: str, scene_number: int) -> str:
        ...
        cmd = [
            "manim", "render",
            "-qm",  # Medium quality (720p30) for clear, readable text
            "--format", "mp4",
            "--media_dir", media_dir,
            "--disable_caching",
