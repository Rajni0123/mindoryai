...
<body class="text-gray-300">
            ...
            <header class="bg-[#0a0a0a] border-b border-gray-800/50 px-6 py-4">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-gray-500 mt-0.5">{{ $dailyChallenge->title }}</p>
                    </div>
                </div>
            </header>
                    ...
                    <div class="bg-white/[0.02] border border-white/5 rounded-lg p-5 space-y-4">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <input type="text" name="title" value="{{ $dailyChallenge->title }}" required
                            ...
                            <div>
                                ...
                                <select name="subject" required
                                    ...
                                    @foreach($subjects as $subject)
                                        <option value="{{ $subject }}" {{ $dailyChallenge->subject === $subject ? 'selected' : '' }}>{{ $subject }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div>
                                ...
                                <select name="difficulty" required
                                        class="w-full px-3 py-2 bg-white/5 border border-white/10 rounded text-white text-sm focus:outline-none focus:border-blue-500">
                                    <option value="easy" {{ $dailyChallenge->difficulty === 'easy' ? 'selected' : '' }}>Easy</option>
                                    <option value="medium" {{ $dailyChallenge->difficulty === 'medium' ? 'selected' : '' }}>Medium</option>
                                    <option value="hard" {{ $dailyChallenge->difficulty === 'hard' ? 'selected' : '' }}>Hard</option>
                                    <option value="mixed" {{ $dailyChallenge->difficulty === 'mixed' ? 'selected' : '' }}>Mixed</option>
                                </select>
                            </div>
                            <div>
                            ...
                            <div class="bg-white/[0.02] border border-white/5 rounded-lg p-4 space-y-3">
                                ...
                                <div>
                                    ...
                                    <textarea x-model="q.question" rows="2"
                                              class="w-full px-3 py-2 bg-white/5 border border-white/10 rounded text-white text-sm focus:outline-none focus:border-blue-500"></textarea>
                                </div>
    ...
    function challengeForm() {
        return {
            questions: @json($dailyChallenge->questions),
            addQuestion() {
