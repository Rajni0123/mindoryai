            ...
            <div class="p-6">
                ...
                <div class="mb-6 border-b border-gray-800/50 overflow-x-auto">
                    <div class="flex gap-4 min-w-max">
                        ...
                        <button onclick="switchTab('credits')" class="tab-btn px-4 py-3 text-sm font-medium text-gray-400 hover:text-blue-400 transition-colors whitespace-nowrap" id="tab-credits">
                            <span class="material-icons-outlined align-middle mr-1" style="font-size: 16px;">account_balance_wallet</span>
                            Credits
                        </button>
                ...
                <!-- Tab Content: Credits -->
                <div id="content-credits" class="tab-content hidden">
                    <form action="{{ route('admin.mobile-app-config.update-credits') }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="card rounded-lg p-6">
                            ...
                            <div class="space-y-6">
                                <div>
                                    <label class="block text-xs font-medium text-gray-400 mb-2">Free Tier Messages</label>
                                    <input type="number" name="free_tier_messages" value="{{ $configs['free_tier_messages'] }}"
                                           min="0" max="500"
                                           class="w-full px-3 py-2 bg-white/5 border border-white/10 rounded text-white text-sm focus:outline-none focus:border-blue-500 transition-colors"
                                           placeholder="50">
                                    <p class="mt-1 text-[10px] text-gray-500">Number of free messages for free users (0-500)</p>
                                </div>

                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div>
                                        <label class="block text-xs font-medium text-gray-400 mb-2">Referral Reward</label>
                                        <input type="number" name="referral_reward" value="{{ $configs['referral_reward'] }}"
                                               min="0" max="100"
                                               class="w-full px-3 py-2 bg-white/5 border border-white/10 rounded text-white text-sm focus:outline-none focus:border-blue-500 transition-colors"
                                               placeholder="3">
                                        <p class="mt-1 text-[10px] text-gray-500">Credits per successful referral</p>
                                    </div>

                                    <div>
                                        <label class="block text-xs font-medium text-gray-400 mb-2">Signup Bonus</label>
                                        <input type="number" name="signup_bonus" value="{{ $configs['signup_bonus'] }}"
                                               min="0" max="100"
                                               class="w-full px-3 py-2 bg-white/5 border border-white/10 rounded text-white text-sm focus:outline-none focus:border-blue-500 transition-colors"
                                               placeholder="3">
                                        <p class="mt-1 text-[10px] text-gray-500">Free credits for new users</p>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="card rounded-lg p-6 mt-6">
                        <h3 class="text-sm font-semibold text-white mb-4 flex items-center gap-2">
                            <span class="material-icons-outlined" style="font-size: 18px;">campaign</span>
                            Referral Statistics
                        </h3>

                        @php
                        $totalReferrals = \App\Models\CreditTransaction::where('type', 'referral')->count();
                        $totalCreditsGiven = \App\Models\CreditTransaction::where('type', 'referral')->sum('amount');
                        $referralsByDate = \App\Models\CreditTransaction::where('type', 'referral')
                            ->selectRaw('DATE(created_at) as date, COUNT(*) as count, SUM(amount) as credits')
                            ->groupBy('date')
                            ->orderBy('date', 'desc')
                            ->limit(10)
                            ->get();
                            ...
                            <div class="p-4 bg-gradient-to-br from-blue-500/10 to-blue-600/10 border border-blue-500/20 rounded-lg">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-xl font-bold text-white">{{ number_format($totalReferrals) }}</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="p-4 bg-gradient-to-br from-green-500/10 to-green-600/10 border border-green-500/20 rounded-lg">
                                <div class="flex items-center gap-3">
                                    <div class="w-10 h-10 rounded-full bg-green-500/20 flex items-center justify-center">
                                        <span class="material-icons-outlined text-green-400" style="font-size: 20px;">coins</span>
                                    </div>
                                    <div>
                                        <p class="text-[10px] text-gray-400 uppercase">Credits Given</p>
                                        <p class="text-xl font-bold text-white">{{ number_format($totalCreditsGiven) }}</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="p-4 bg-gradient-to-br from-purple-500/10 to-purple-600/10 border border-purple-500/20 rounded-lg">
                                <div class="flex items-center gap-3">
                                    <div class="w-10 h-10 rounded-full bg-purple-500/20 flex items-center justify-center">
                                        <span class="material-icons-outlined text-purple-400" style="font-size: 20px;">calendar_today</span>
                                    </div>
                                    <div>
                                        <p class="text-[10px] text-gray-400 uppercase">This Month</p>
                                        <p class="text-xl font-bold text-white">
                                            {{ number_format(\App\Models\CreditTransaction::where('type', 'referral')->whereMonth('created_at', now()->month)->count()) }}
                                        </p>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div>
                            <h4 class="text-xs font-medium text-gray-400 mb-3">Recent Referrals (Date-wise)</h4>
                            @if($referralsByDate->count() > 0)
                                <div class="overflow-x-auto">
                                    <table class="w-full text-sm">
                                        <thead>
                                            <tr class="border-b border-white/10">
                                                <th class="text-left py-2 px-3 text-[10px] font-medium text-gray-400 uppercase">Date</th>
                                                <th class="text-center py-2 px-3 text-[10px] font-medium text-gray-400 uppercase">Referrals</th>
                                                <th class="text-center py-2 px-3 text-[10px] font-medium text-gray-400 uppercase">Credits Given</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            @foreach($referralsByDate as $stat)
                                            <tr class="border-b border-white/5 hover:bg-white/5">
                                                <td class="py-2 px-3 text-white text-xs">{{ \Carbon\Carbon::parse($stat->date)->format('M d, Y') }}</td>
                                                <td class="py-2 px-3 text-center text-white text-xs">{{ number_format($stat->count) }}</td>
                                                <td class="py-2 px-3 text-center text-green-400 text-xs">+{{ number_format($stat->credits) }}</td>
                                            </tr>
                                            @endforeach
                                        </tbody>
                                    </table>
                            ...
                            @else
                                <div class="p-4 bg-gray-500/10 border border-gray-500/20 rounded-lg text-center">
                                    <p class="text-xs text-gray-400">No referrals yet</p>
                                </div>
                            @endif
                        </div>
                    </div>
