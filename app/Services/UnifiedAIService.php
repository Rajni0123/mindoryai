...
class UnifiedAIService
{
     ...
     * Returns: 'free', 'lite', 'pro', 'ultimate', or 'free' as default
     *
    ...
    protected function getUserPlanSlug(?int $userId): string
    {
        ...
        try {
            ...
            if (!$user || !$user->plan_id) {
                self::$userPlanSlugCache[$userId] = 'free';
                return 'free';
            }

            $plan = $user->userPlan;
            $slug = $plan ? ($plan->slug ?? 'free') : 'free';
     ...
     */
    protected function selectModel(string $question, string $feature, string $planSlug, string $preferredProvider = 'google'): array
    {
        // Define model tiers for each provider
        $modelTiers = [
            'google' => [
                'basic' => 'gemini-2.0-flash',           // Default/basic model
                'fast' => 'gemini-2.0-flash-lite',       // Cheap and fast for simple questions
                'best' => 'gemini-1.5-pro',              // Best quality for complex questions
            ],
            'openai' => [
                'basic' => 'gpt-4o-mini',                // Default/basic model
                'fast' => 'gpt-4o-mini',                 // Same as basic for OpenAI
                'best' => 'gpt-4o',                      // Best quality for complex questions
            ],
        ];

        // Free and Lite plans: always use basic model (no adaptive switching)
        if (in_array($planSlug, ['free', 'lite', 'starter'])) {
            $tier = 'basic';
            $modelId = $modelTiers[$preferredProvider][$tier] ?? $modelTiers['google']['basic'];

            Log::debug('Model selection (basic plan)', [
                'plan' => $planSlug,
                'model' => $modelId,
        ...
        // Pro and Ultimate plans: adaptive model switching
        $isComplex = $this->isQuestionComplex($question, $feature);
        $tier = $isComplex ? 'best' : 'fast';
        $modelId = $modelTiers[$preferredProvider][$tier] ?? $modelTiers['google']['basic'];

        Log::info('Adaptive model selection', [
            'plan' => $planSlug,
    ...
    protected function callWithFailover(
        ...
        // Determine if failover is enabled based on plan
        $failoverEnabled = !in_array($planSlug, ['free']);

        // Select primary model based on complexity (if adaptive is enabled for plan)
        $primarySelection = $this->selectModel($message, $feature, $planSlug, 'google');
        ...
        // FAILOVER: Try backup provider (OpenAI)
        ...
        $backupSelection = $this->selectModel($message, $feature, $planSlug, 'openai');
        ...
        // Fallback to any active OpenAI model
        if (!$backupModel) {
            $backupModel = AiModel::where('provider', 'openai')
                ->where('is_active', true)
                ->orderBy('order')
                ->first();
        }
    ...
    {
        // Store language for use in system prompt
...
REMEMBER: You are BLINKY - make learning VISUAL and FUN with icons!";
    ...
    {
        // Build language instruction based on current preference
