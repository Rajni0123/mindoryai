...
class TokenTrackingService
{
    ...
    public function trackUsage(int $userId, int $tokensUsed, string $planType): array
    {
        ...
        // For FREE users, track lifetime tokens
        if ($planType === 'free') {
            DB::table('users')
                ->where('id', $userId)
                ->increment('lifetime_tokens_used', $tokensUsed);

            $user = DB::table('users')->where('id', $userId)->first();
            return [
                'questions_today' => $this->getQuestionsToday($userId),
                'lifetime_tokens_used' => $user->lifetime_tokens_used,
                'lifetime_tokens_remaining' => 50000 - $user->lifetime_tokens_used
            ];
        }

        // For paid users, track monthly tokens
    ...
    public function getUsageStatus(int $userId, string $planType): array
    {
        ...
        $dailyLimit = $dailyLimits[$planType] ?? 5;
        ...
        if ($planType === 'free') {
            ...
            return [
                ...
                'lifetime_tokens_used' => $user->lifetime_tokens_used ?? 0,
                'lifetime_tokens_limit' => 50000,
                'lifetime_tokens_remaining' => 50000 - ($user->lifetime_tokens_used ?? 0)
            ];
        }
