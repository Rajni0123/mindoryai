...
class StudyProfileCatalog
{
    ...
    public static function isAutoGeneratedEmail(?string $email): bool
    {
        ...
        if (str_contains($e, '@mobile.')) {
            return true;
        }
        if (preg_match('/^\d{10}_\d+@/', $e)) {
            return true;
        }
        if (preg_match('/^\d{10}@mobile/', $e)) {
            return true;
        }
    ...
    {
        if (self::isAutoGeneratedEmail($email)) {
            return null;
        }
