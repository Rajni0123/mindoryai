...
GIVE THE DETAILED EXPLANATION NOW.";
    ...
    public function solveFromImage(Request $request)
    {
        ...
        $planSlug = $user?->userPlan?->slug;
        $isPremium = in_array($planSlug, ['ultimate', 'pro', 'starter']);

        // GPT-4o vision first (accuracy), Gemini fallback — PDF uses Gemini
        try {
            $imageData = [
            ...
            $isPdf = $imageData['type'] === 'application/pdf';
            ...
            $timeout = $isPremium ? 45 : 35;
            $response = null;
            $usedProvider = null;

            if (!$isPdf) {
                try {
                    $response = $this->solveImageWithOpenAI(
                    ...
                    Log::warning('OpenAI vision failed, trying Gemini', ['error' => $openAiError->getMessage()]);
                }
            }
            ...
            return response()->json([
                ...
                'model' => $response['model'] ?? config('ai.vision_model', 'gpt-4o'),
                'fast_mode' => true,
            ]);
    ...
    /**
     * Scan/solve with OpenAI GPT-4o vision (primary — high accuracy).
     */
    private function solveImageWithOpenAI(
        string $imageBase64,
        string $mimeType,
        string $prompt,
        int $maxTokens,
        ...
        $apiKey = \App\Models\FrontendConfig::getValue('ai.openai_api_key', '')
            ?: config('ai.openai.api_key', env('OPENAI_API_KEY', ''));

        if (empty($apiKey)) {
            throw new \Exception('OpenAI API key not configured');
        }

        $model = config('ai.vision_model', 'gpt-4o');
        ...
        $response = \Illuminate\Support\Facades\Http::withHeaders([
            ...
            ->post('https://api.openai.com/v1/chat/completions', [
                ...
                'messages' => [
                    [
                        'role' => 'system',
                        'content' => 'You are BlinkStudy AI tutor for Indian students (CBSE/JEE/NEET). Solve accurately with clear step-by-step steps. Use Hinglish when helpful.',
                    ],
                    [
                        'role' => 'user',
                        'content' => [
        ...
        if (empty(trim($content))) {
            throw new \Exception('OpenAI vision returned empty content');
        }
