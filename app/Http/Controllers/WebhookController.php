...
class WebhookController extends Controller
{
    public function __construct(
        protected TrialAutopayService $trialAutopayService
    ) {}
    ...
    public function razorpay(Request $request)
    {
        ...
        // Subscription autopay (₹1 trial) events
        if ($event && str_starts_with($event, 'subscription.')) {
            try {
                $this->trialAutopayService->handleWebhookEvent($event, $request->all());
            } catch (\Exception $e) {
                Log::error('Trial autopay webhook failed', [
                    'event' => $event,
                    'error' => $e->getMessage(),
                ]);
            }

            return response()->json(['status' => 'subscription_processed']);
        }
